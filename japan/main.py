...
def model_findout(page_url, page_request_headers, page_xpath_expreesion,
    ...
    while model_findout is False and err_status <= 10:
        try:
            ...
            logger.debug('在网页 ' + page_url + ' 中遍历 日本妹子 时，出现错误。睡眠 ' +
                         str(random_int) + ' 秒钟。')
            time.sleep(random_int)
    else:
        if model_findout is True:
            logger.info('########################')
            logger.info('网页 ' + page_url + ' 中的 日本妹子 遍历成功。')
        if err_status == 10:
...
if __name__ == '__main__':
    # 定义目录前缀
    dir_prefix = 'E:\\妹子图\\日本妹子\\'
    # 配置日志
    log_file = dir_prefix + 'log.txt'
    logger = logger(log_file=log_file, log_console=True)

    # 遍历 日本妹子 的所有页面
    for page_url_index in range(1, 29 + 1):
        page_url_prefix = 'https://www.mzitu.com/japan/page/'
        page_url_suffix = '/'
        ...
        while job_finished is False and err_status1 <= 10:
            try:
                ...
                err_status1 += 1
                random_int = random.randint(30, 60)
                logger.debug('搜索 日本妹子 出现异常。睡眠 ' + random_int + ' 秒钟。')
                time.sleep(random_int)
